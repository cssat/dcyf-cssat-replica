...
WITH child_removal_episode AS (
	...
	CASE WHEN "CD_First_Visit_Attended_Modality" = 1 THEN 'Virtual' ELSE 'In Person' END AS "First_Visit_Modality",
	"Total_Referrals",
	...
	ELSE 0 END AS "CD_Outcome_72_Hour_Visit",
	...
	FIRST_VALUE(outcome_72_hour_visit) OVER (PARTITION BY "ID_Child_Removal_Episode" ORDER BY "FL_Referral_72_Hour" DESC, "ID_Referral") 
	AS "First_Outcome_72_Hour_Visit"
	FROM (
		SELECT *,
		CASE WHEN "ID_First_Visit" = MIN("ID_First_Visit") OVER (PARTITION BY "ID_Child_Removal_Episode") 
		THEN "ID_Referral" ELSE NULL END AS id_first_referral,
		CASE WHEN "ID_First_Visit" = MIN("ID_First_Visit") OVER (PARTITION BY "ID_Child_Removal_Episode") 
		THEN "DT_Start" ELSE NULL END AS dt_first_start,
		CASE WHEN "ID_First_Visit" = MIN("ID_First_Visit") OVER (PARTITION BY "ID_Child_Removal_Episode") 
		THEN "CD_First_Visit_Modality" ELSE NULL END AS cd_first_visit_modality,
		CASE WHEN "ID_Referral" = MAX("ID_Referral") OVER (PARTITION BY "ID_Child_Removal_Episode") 
		...
		CASE WHEN "ID_Referral" = MIN("ID_Referral") OVER (PARTITION BY "ID_Child_Removal_Episode") 
		...
		ORDER BY "ID_Child_Removal_Episode", "ID_Referral", "ID_First_Visit") cre
	) crex
	WHERE "ID_Referral" = "ID_First_Referral"
	ORDER BY "ID_Person", "DT_Coalesced"
)
